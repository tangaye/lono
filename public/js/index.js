...
const chatView = {
            ...
            return `
                <li id='message-${message.id}' class='message-block'>
                    <article class="message">
                        <header class="sender">${
                          user.name === message.sender ? "you" : message.sender
                        }</header>
                        <p>${message.content}</p>
                    </article>
                    <div class="message-status">
                        <time class="time">${moment(message.date).format(
                          "h:mma"
                        )}</time>
                        <span class="status">${
                          user.name === message.sender ? message.status : ""
                        }</span>
                    </div>
                </li>
            `;
    ...
    displayMessage(message) {
        pageEl.conversationEl.insertAdjacentHTML(
            "beforeend",
    ...
    displaySentMessages(messages) {
        ...
        if (user) {
            messages.forEach(message => {
                ...
                if (message.status !== READ && message.sender !== user.name)
                        ...
                        <div class="user-details">
                            ...
                            <span class="last-seen">
                                ${
                                  user.status === OFFLINE
                                    ? "last seen " +
                                      moment(user.lastSeen).format("h:mma")
                                    : ""
                                }
                            </span>
                        </div>
    ...
    setupEventListeners() {
        ...
        document.addEventListener('click', event => {
            ...
            if (event.target.classList.contains('open-sidebar')) openSidebar();
            if (event.target.classList.contains('close-sidebar')) closeSidebar();
        });
...
client.on("connected", (user, users, messages) =>
    chatController.userReconnect(user, users, messages)
);

client.on("logged in", (username, users, messages) =>
    chatController.userLoggedIn(username, users, messages)
);
...
client.on("user details", (user, users, messages) =>
    chatController.setupUser(user, users, messages)
);
...
const openSidebar = () => {
    ...
    document.querySelector('#sidebar').style.padding = "15px";
}
...
// Display default sidebar when tablet or desktop size
